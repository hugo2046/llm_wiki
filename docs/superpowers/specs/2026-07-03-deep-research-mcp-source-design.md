...
5. 中英 i18n 文案（`settings.sections.webSearch.mcp*`，`i18n-parity.test.ts` 强制键对齐）。
...
- 编辑字段：name、url、authHeader（密码型输入）、toolName、queryParam
  （默认占位 "query"）、extraArgs（JSON 输入，非法时字段下方红字提示，
  不阻断保存——避免输入途中丢内容；research 时该 server 报错进 errors）。
